...
export class UserService {
  ...
  async generateNonce(walletAddress: string): Promise<string> {
    ...
    const nonce = randomBytes(16).toString('hex'); // 32 字符 hex nonce
    user.nonce = nonce;
  ...
  private async findByWallet(wallet: string): Promise<User | null> {
    return this.userRepository.findOne({ where: { walletAddress: wallet } });
  }
}
